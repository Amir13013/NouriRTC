...
export const createUser = async (req, res, next) => {
  try {
    ...
    const newUser = await createUserService(name, first_name, phone_number, mail, hashedPassword);
...
export const login = async (req, res) => {
  try {
    ...
    return res.status(200).json({
      ...
      user: { id: user.id, mail: user.mail, name: user.name, first_name: user.first_name },
    });
  } catch (error) {
    console.error(error);
