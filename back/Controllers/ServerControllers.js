...
const disconnectUserFromServer = async (io, userId, serverId, eventName) => {
  ...
  for (const s of sockets) {
    if (String(s.data?.userId) === String(userId)) {
      for (const channelId of channelIds) await s.leave(channelId);
...
export const kickUserFromServer = async (req, res, next) => {
  try {
    ...
    if (targetRole.role === 'owner') return handleResponse(res, 403, "Cannot kick the owner");
    ...
    const io = req.app.get('io');
    if (io) await disconnectUserFromServer(io, userId, serverId, 'member:kicked');
...
export const banUserFromServer = async (req, res, next) => {
  try {
    ...
    if (alreadyBanned) return handleResponse(res, 400, "User is already banned from this server");
    const targetRole = await getUserRoleInServerService(serverId, userId);
    if (targetRole) {
      if (targetRole.role === 'owner') return handleResponse(res, 403, "Cannot ban the owner");
      if (targetRole.role === 'admin' && requesterRole !== 'owner')
    ...
    if (targetRole) {
      const io = req.app.get('io');
      if (io) await disconnectUserFromServer(io, userId, serverId, 'member:banned');
    }
